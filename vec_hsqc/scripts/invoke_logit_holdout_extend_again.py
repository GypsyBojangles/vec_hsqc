...
Xtr_abr = np.abs( abridge_features( X_train ) )
Xcv_abr = np.abs( abridge_features( X_CV ) )

X_train = np.abs( np.hstack( [ X_train[:,1:-2], X_train[:,-1].reshape( X_train.shape[0], 1) ] ) )
X_CV = np.abs( np.hstack( [ X_CV[:,1:-2], X_CV[:,-1].reshape( X_CV.shape[0], 1) ] ) )
#X_CV = np.abs( X_CV[:, 1:] )
