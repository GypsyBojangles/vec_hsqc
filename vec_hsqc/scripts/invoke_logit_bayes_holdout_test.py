...
if __name__ == '__main__':
    ...
    X1, lm1, Fsp1 = wg.remove_assigned( a1.y_pred, legmat, [ a1.X, legmat, a1.Fsp ] )
    X2, lm2, Fsp2, y_crude = wg.rapid_wild_guess( X1, lm1, Fsp1 )
    a2 = post_proc.DataOut()
    y_pred_combined = np.concatenate( a1.y_pred, y_crude )
    legmat_combined = np.vstack( [ legmat, lm2 ] )
    csarray_combined = np.vstack( [ csarray_test, Fsp2 ] )
    a2.generate_master_peak_list( y_pred_combined, legmat_combined, csarray_combined )
    #np.savetxt( os.path.join( curdir, 'master_array_log_nearest.npy' ), a2.master_array, fmt="%s" )
    a2.writeall_peak_lists( a2.master_array, curdir, 'dual_prediction_new_' )
